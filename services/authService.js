...
class AuthService{
    ...
    async generateToken(user , expiry='1h' , type = 'access'){
        ...
        if(type == 'refresh') 
            return jwt.sign({ userId: user.userId || user._id ,name: user.name, displayName: user.displayName, email: user.email, type: type, avatar: user.avatar , coins: user.coins }, this.refreshSecret, { expiresIn:expiry });
        return jwt.sign({ userId: user.userId || user._id,name: user.name, displayName: user.displayName, email: user.email, type: type , avatar: user.avatar, coins: user.coins }, this.accessSecret, { expiresIn:expiry });
    }
