...
class FriendController {
    ...
    async acceptRequest(req, res) {
        ...
        try {
            ...
            await notifications.findOneAndUpdate({ _id, userId: req.user._id, type }, { status: 'read' });


            // Update the friendShip status
            const friendShipData = await friendShip.findOneAndUpdate({ recipent: req.user._id, requestId: requestId._id }, { status: 'active' }, { new: true });

            const requester = await userRepo.findUserById(friendShipData.requester);

            // Update Request status
            await Requests.findOneAndUpdate({ _id: requestId._id, recipent: req.user._id }, { status: 'Approved' })

            // Publish notification
            await notificationService.send(requester.email, req.user.displayName, `Accepted your friend request!`, 'info');
    ...
    async rejectRequest(req, res) {
        ...
        try {
            ...
            // Update the friendShip status
            await friendShip.findOneAndDelete({ recipent: req.user._id, requestId: requestId._id })

            // Update Request status
            await Requests.findOneAndUpdate({ _id: requestId._id, recipent: req.user._id }, { status: 'Rejected' })

            // Publish notification
            await notificationService.send(req.user.email, req.displayName.name, `Decline your friend request!`, 'info');
    ...
    async getFriends(req, res) {
        ...
        const friends = friendships.map(f => {
            ...
            return requester._id.toString() === _id.toString() ? recipient : requester;
        });
